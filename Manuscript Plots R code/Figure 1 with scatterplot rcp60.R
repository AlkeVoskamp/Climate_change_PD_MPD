...
#   scale_fill_gradientn("% change",colours=c(re1,wi1,bl1),limits=c(-80,275),breaks=c(-80,275),labels=c("-80","275"))+ # Insert colour and set range
...
#   geom_raster(aes(fill =  SRpropChange), stat = "identity", position = "identity", hjust = 0, vjust = 0, interpolate = FALSE,)+
#   scale_fill_gradientn("% change",colours=c(re1,wi1,bl1),limits=c(-14,54),breaks=c(-14,54),labels=c("-14","54"))+ # Insert colour and set range
#   theme(legend.position = c(0.5, 0.6))+ # Positioning the legend <- needs to run with legend position to extract legend  
#   theme(legend.text=element_text(size=19),legend.title=element_text(size=23))+ # Change font size legend
#   theme(legend.key.size = unit(0.5, "cm"),legend.key.height = unit(0.5, "cm")) # Change size of legend key
# 
...
Final <- arrangeGrob(CombScatter,CombPlots,
                     widths = c(5),
                     heights = c(1,1),
                     ncol = 1,
                     nrow = 2)
...
ggsave("Figure 1 Changes SR PD MPD rcp60_colour.tiff",Final,width=24, height=9, unit="in", dpi=600, bg="transparent")
