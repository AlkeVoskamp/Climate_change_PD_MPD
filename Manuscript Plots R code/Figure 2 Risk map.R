...
RPDchange <- read.csv("PDandMPD_random_changes_disp2_rcp60_2080_RC.csv")
head(RPDchange)
str(RPDchange)

## Missing cells
Mdata <- read.csv("E:/PD analysis/Second analysis non random loss win/Result files random PD change/PD_SR_MPD_values_missing_cells_to_complete_maps_Disp2_rcp60_80.csv")
...
SRscatter <- ggplot(RPDchange,aes(x=PDpropChange,y=MPDpropChange,col=Risk_categories))+
...
ggsave("Figure 3 PD vs MPD change map rcp60.tiff",Final,width=24, height=8, unit="in", dpi=600, bg="transparent")
...
FinalComb <- grid.arrange(
              ...
              arrangeGrob(BarPlot,
                          ...
                          ncol = 1,
                          nrow = 1),
              nrow=2, ncol =1, heights=c(2,8), width=c(12)
)
plot(FinalComb)


setwd("E:/PD analysis/PD manuscript/PD Manuscript Figures/")
ggsave("Figure 3 PD vs MPD change map rcp60_with BC.tiff",FinalComb,width=24, height=10, unit="in", dpi=600, bg="transparent")
