...
bl1 <- colorRampPalette(rev(c("lightskyblue","royalblue","navy")))(543) #553                     
re1 <- colorRampPalette(rev(c("darkred","red2","mistyrose")))(666)      #538
...
plot1<- ggplot(data=PDRLoss, aes(y=y, x=x)) +
  ...
  scale_fill_gradientn("",colours=c(bl1,wi1,re1),limits=c(-544,668))+ # Insert colour and set range
...
bl1 <- colorRampPalette(rev(c("lightskyblue","royalblue","navy")))(543)  #931.3    553                
...
bl1 <- colorRampPalette(rev(c("navy","royalblue","lightskyblue")))(817)  #931.3                    
re1 <- colorRampPalette(rev(c("mistyrose","red2","darkred")))(1170)       #1026.5
wi1 <- colorRampPalette(rev(c("gray82","gray90","gray82")))(4)
...
plot2<- ggplot(data=PDRGain, aes(y=y, x=x)) +
  ...
  scale_fill_gradientn("",colours=c(re1,wi1,bl1),limits=c(-1170,817))+ # Insert colour and set range
  borders("world",  xlim = c(-180, 180), ylim = c(-60,90), lwd=0.3, colour ="black")+
  geom_segment(aes(x=-180,xend=180,y=0,yend=0),colour="black",linetype="dashed")+ #Add shortened equator line 
  theme(legend.position = "none")+ # Positioning the legend 
  theme(axis.title=element_text(size=28))+ # Change font size legend
...
bl1 <- colorRampPalette(rev(c("lightskyblue","royalblue","navy")))(77) #1137                     
re1 <- colorRampPalette(rev(c("darkred","red2","mistyrose")))(36)      #468
...
plot3<- ggplot(data=MPDRLoss, aes(y=y, x=x)) +
  ...
  scale_fill_gradientn("",colours=c(bl1,wi1,re1),limits=c(-77,36))+ # Insert colour and set range
  borders("world",  xlim = c(-180, 180), ylim = c(-60,90), lwd=0.3, colour ="black")+
  geom_segment(aes(x=-180,xend=180,y=0,yend=0),colour="black",linetype="dashed")+ #Add shortened equator line 
  theme(legend.position = "none")+ # Positioning the legend 
  theme(axis.title=element_text(size=28))+ # Change font size legend
...
bl1 <- colorRampPalette(rev(c("lightskyblue","royalblue","navy")))(77)  #931.3                    
re1 <- colorRampPalette(rev(c("darkred","red2","mistyrose")))(36)       #1026.5
wi1 <- colorRampPalette(rev(c("gray82","gray90","gray82")))(0)
...
re1 <- colorRampPalette(rev(c("mistyrose","red2","darkred")))(20)       #1026.5
wi1 <- colorRampPalette(rev(c("gray82","gray90","gray82")))(0)
...
plot4 <- ggplot(data=MPDRGain, aes(y=y, x=x)) +
  ...
  scale_fill_gradientn("",colours=c(re1,wi1,bl1),limits=c(-20,33))+ # Insert colour and set range
  borders("world",  xlim = c(-180, 180), ylim = c(-60,90), lwd=0.3, colour ="black")+
  geom_segment(aes(x=-180,xend=180,y=0,yend=0),colour="black",linetype="dashed")+ #Add shortened equator line 
  theme(legend.position = "none")+ # Positioning the legend 
  theme(axis.title=element_text(size=28))+ # Change font size legend
...
ggsave("Figure 4 Random_PD and_MPD_change_rcp60_significant_cells.tiff",CombNull,width=35, height=14, unit="in", dpi=300, bg="transparent")
