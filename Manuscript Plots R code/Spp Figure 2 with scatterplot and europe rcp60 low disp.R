...
#-#-#-#-#-#-#-#-#-#-#-#-#-#-# Plot Spp version of Figure 2 #-#-#-#-#-#-#-#-#-#-#-#-#-#-#

#-#-# Load plot data and add PD change columns #-#-#
setwd("/Users/alkevoskamp/Documents/PD manuscript/Data for plots/")
CombCon <- read.csv("PDandMPD_random_changes_disp1_rcp60_2080_Final.csv")
...
# ConData <- get(load("/Users/alkevoskamp/Documents/PD manuscript/Continents_gridded_05.Rdata"))
# head(ConData)
...
# ## Save the final file
# setwd("/Users/alkevoskamp/Documents/PD manuscript/Data for plots/")
# write.csv(CombCon,"PDandMPD_random_changes_disp1_rcp60_2080_Final.csv")
...
bl1 <- colorRampPalette(rev(c("#08306B","#08519C","#2171B5","#4292C6","#9ECAE1","#DEEBF7")))(245) 
re1 <- colorRampPalette(rev(c("#FFEDA0","#FEB24C","#FC4E2A","#E31A1C","#800026")))(73) 
...
SRplot<- ggplot(data=CombCon, aes(y=y, x=x)) +
  ...
  scale_fill_gradientn("SR\n%change",colours=c(re1,wi1,bl1),limits=c(-75,247))+ # Insert colour and set range #-72,100
...
SREurope <- ggplot(data=CombCon, aes(y=y, x=x)) +
  geom_raster(aes(fill =  SRpropChange), stat = "identity", position = "identity")+
  scale_fill_gradientn("SR\n%change",colours=c(re1,wi1,bl1),limits=c(-75,247))+ 
...
bl1 <- colorRampPalette(rev(c("#08306B","#08519C","#2171B5","#4292C6","#9ECAE1","#DEEBF7")))(171) 
re1 <- colorRampPalette(rev(c("#FFEDA0","#FEB24C","#FC4E2A","#E31A1C","#800026")))(60) 
wi1 <- colorRampPalette(rev(c("#FFFFCC")))(4)

PDplot<- ggplot(data=CombCon, aes(y=y, x=x)) +
  geom_raster(aes(fill =  PDpropChange), stat = "identity", position = "identity")+
  scale_fill_gradientn("PD\n%change",colours=c(re1,wi1,bl1),limits=c(-62,173))+ # Insert colour and set range #-72,100
  borders("world",  xlim = c(-180, 180), ylim = c(-60,90), lwd=0.3, colour ="black")+
  geom_segment(aes(x=-180,xend=180,y=0,yend=0),colour="black",linetype="dashed")+ #Add shortened equator line 
  theme(legend.position = c(0.1, 0.2))+ # Positioning the legend 
  theme(axis.title=element_text(size=28))+ # Change font size legend
...
PDEurope <- ggplot(data=CombCon, aes(y=y, x=x)) +
  geom_raster(aes(fill =  PDpropChange), stat = "identity", position = "identity")+
  scale_fill_gradientn("PD\n%change",colours=c(re1,wi1,bl1),limits=c(-62,173))+
  borders("world",  xlim = c(-10, 40), ylim = c(35,70), lwd=0.3, colour ="black")+
  coord_cartesian(xlim = c(-10, 40), ylim = c(35, 70))+ #Set the plot extent
  geom_segment(aes(x=-180,xend=180,y=0,yend=0),colour="black",linetype="dashed")+ #Add shortened equator line 
  theme(legend.position = "none")+ # Positioning the legend 
...
bl1 <- colorRampPalette(rev(c("#08306B","#08519C","#2171B5","#4292C6","#9ECAE1","#DEEBF7")))(33)  
re1 <- colorRampPalette(rev(c("#FFEDA0","#FEB24C","#FC4E2A","#E31A1C","#800026")))(20) 
...
MPDplot<- ggplot(data=CombCon, aes(y=y, x=x)) +
  ...
  scale_fill_gradientn("MPD\n%change",colours=c(re1,wi1,bl1),limits=c(-21,34))+ # Insert colour and set range #-72,100
  borders("world",  xlim = c(-180, 180), ylim = c(-60,90), lwd=0.3, colour ="black")+
  geom_segment(aes(x=-180,xend=180,y=0,yend=0),colour="black",linetype="dashed")+ #Add shortened equator line 
  theme(legend.position = c(0.1, 0.2))+ # Positioning the legend 
  theme(axis.title=element_text(size=28))+ # Change font size legend
...
MPDEurope <- ggplot(data=CombCon, aes(y=y, x=x)) +
  geom_raster(aes(fill =  MPDpropChange), stat = "identity", position = "identity")+
  scale_fill_gradientn("MPD\n%change",colours=c(re1,wi1,bl1),limits=c(-21,34))+
  borders("world",  xlim = c(-10, 40), ylim = c(35,70), lwd=0.3, colour ="black")+
  coord_cartesian(xlim = c(-10, 40), ylim = c(35, 70))+ #Set the plot extent
  geom_segment(aes(x=-180,xend=180,y=0,yend=0),colour="black",linetype="dashed")+ #Add shortened equator line 
  theme(legend.position = "none")+ # Positioning the legend 
...
ggsave("Spp Figure 2 Changes SR PD MPD rcp60_lowDisp_europe_Final.tiff",Final,width=18, height=13, unit="in", dpi=600, bg="transparent")
